...
class ComOhanahTemplateHelperPaginator extends ComDefaultTemplateHelperPaginator
{
    public function pagination($config = array())
    {
        ...
        if (count($pages['pages']) > 1) {
            ...
            if ($config->current > 1) {
                $html .= '<a href="'.$this->_getPageURL($pages['previous']).'" class="button" disabled>'.JText::_('OHANAH_PAGINATOR_PREVIOUS_PAGE').'</a>';    
            }
            
            $html .= $this->_pages($pages, $config->count, $config->current, $config->last);

            if ($config->current < $config->count) {
                $html .= '<a href="'.$this->_getPageURL($pages['next']).'" class="button" disabled>'.JText::_('OHANAH_PAGINATOR_NEXT_PAGE').'</a>';    
            }
