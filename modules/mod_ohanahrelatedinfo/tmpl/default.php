...
<div class="ohanah_module">
	...
	<? if ($event->latitude && $event->longitude) : ?>
		...
		<? if ($params->get('showWikipedia')) : ?>
			<a target="blank" class="link" href="http://wikipedia.org/wiki/<?=$event->geolocated_city ?>"><?=@text('ON_WIKIPEDIA')?></a><br />
		<? endif ?>
		
		<? if ($params->get('showTripadvisor')) : ?>
			<a target="blank" class="link" href="http://www.tripadvisor.org/<?=$event->geolocated_city ?>"><?=@text('GET_TRIP_ADVICE')?></a><br />
		<? endif ?>
		
		<? if ($params->get('showPanoramio')) : ?>
			<a target="blank" class="link" href="http://www.panoramio.com/map/#lt=<?=$event->latitude ?>&ln=<?=$event->longitude ?>&z=4&k=2"><?=@text('OHANAH_SEE_ON_PANORAMIO')?></a>
		<? endif ?>
