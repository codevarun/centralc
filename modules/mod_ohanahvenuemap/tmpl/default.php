...
<div class="ohanah module<?php echo $params->get( 'moduleclass_sfx' ) ?>">
	<? if ($venue['latitude'] && $venue['longitude']) : ?>
		<div id="event_main_map_wrapper">
			...
			<div id="event_main_map" style="width: <?=$width?>px; height: <?=$height?>px; background: url(http://maps.google.com/maps/api/staticmap?zoom=13&size=<?=$width?>x<?=$height?><? if (substr($_SERVER['HTTP_HOST'], 0, 9) != 'localhost') echo '&markers=icon:http://'.$_SERVER['HTTP_HOST'].KRequest::base().'/media/com_ohanah/images/ohanah.png'; else echo '&markers=color:blue'; ?>|<?=$venue['latitude']?>,<?=$venue['longitude']?>&sensor=false) no-repeat;">
